...
ROI_DISPLAY = True
FPS = 30
BUFFER_SIZE = int(FPS * 15)  # 15s buffer = faster yet stable
BPM_SMOOTHING_WINDOW = 10    # average BPM from last 10 readings
...
print(f"[INFO] Webcam initialized successfully at 640x480 (FPS={FPS})")
...
while True:
    ...
    if len(boxes) == 0:
        ...
        cv2.imshow("Heart Rate Detector (Webcam 640x480)", frame)
    ...
    # Left under-eye (slightly below eye)
    ...
    if len(forehead_buffer) >= SMOOTH_WINDOW:
        # Weighted ROI fusion (forehead dominant)
        ...
        if len(signal) >= BUFFER_SIZE - SMOOTH_WINDOW + 1:
            ...
            if np.any(idx):
                ...
                bpm_display = int(np.mean(bpm_history))  # smooth BPM
    ...
    elif len(bpm_history) >= 5 and np.std(bpm_history) < 5:
        cv2.putText(frame, "Stable", (500, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    else:
        cv2.putText(frame, "Unstable", (500, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
...
# =========================================
# 📊 OPTIONAL: PLOT THE FINAL FILTERED SIGNAL
# =========================================
try:
    signal = (0.6 * np.array(forehead_buffer)
             + 0.2 * np.array(left_eye_buffer)
             + 0.2 * np.array(right_eye_buffer))
    signal = (signal - np.mean(signal)) / np.std(signal)
    signal = bandpass_filter(signal, fs=FPS)
    ...
    plt.title(f"rPPG Signal (Filtered) – FPS: {FPS}")
    plt.xlabel("Frames")
